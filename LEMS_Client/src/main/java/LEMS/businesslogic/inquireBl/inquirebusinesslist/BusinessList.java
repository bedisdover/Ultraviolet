...
public class BusinessList {
	...
	public ArrayList<PayBillVO> getPayBill(){
		...
		for(int i=0;i<pay.size();i++){
			PayBillVO pav=new PayBillVO(pay.get(i).getDate(),pay.get(i).getId(),pay.get(i).getInstitution(),pay.get(i).getAmount(),pay.get(i).getPayer(),pay.get(i).getAccount(),pay.get(i).getItem(),pay.get(i).getRemark());
			pv.add(pav);
		}
		return pv;
	}
